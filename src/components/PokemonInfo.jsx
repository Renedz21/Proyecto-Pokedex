        ...
        <Link to={`/pokemon/${pokemonInfo.id}`}>
            <div className={`shadow-lg h-full rounded-lg bg-${pokemonInfo.types[0].type.name}`}>
                <div className='flex items-center justify-between py-2 px-4'>
                    <h2 className='text-xl font-bold capitalize text-white '>{pokemonInfo.name}</h2>
                ...
                <div className='flex items-center justify-between p-2'>
                    <div className='flex flex-col gap-2 justify-start'>
                        {pokemonInfo.types.map((type, index) => (
                            <span key={index}
                                className={`font-semibold text-white px-3 py-1 capitalize text-sm rounded-full bg-light-${type.type.name}`}>
                                {type.type.name}
                            </span>
                        ))}
                    </div>
